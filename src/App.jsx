...
import chapters from '../reference/swift/chapters.json'

const _require = require.context("../reference/swift", true, /html$/)

const htmls = {}
chapters.forEach(function({ title: chapter, details }, i) {
    details.forEach(function(detail, i) {
        const file = `./${chapter}/${detail}.html`

        const html = _require(file)
        let titles = htmls[chapter] || {}
        titles[detail] = html
        htmls[chapter] = titles
    })
})
  ...
  render() {
    return (
      <div className="App">
        ...
        {chapters.map(function({title, details}, i){
          
          return <div key={i}>
            <h2 className="chapter-name">{title}</h2>
            {details.map(function(detail, i){
              return <div key={i}>
                <h3 className="section-name">{detail}</h3>
                <div key={i} className="code" dangerouslySetInnerHTML={{__html: htmls[title][detail]}}></div>
              </div>
            })}
          </div>
        })}
